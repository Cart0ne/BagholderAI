...
4. **Crash scenario:** quando `proposed_stop_buy_active = true` (risk_score > 90), analizziamo: i buy effettuati dal Grid in quel periodo si sono rivelati profittevoli o no? Se no → in Sprint 2 promuoviamo `stop_buy_drawdown_pct` a parametro gestito da Sherpa
5. L'analisi la facciamo noi (CEO) via SQL dopo ~7 giorni di dati

### `stop_buy_drawdown_pct` — policy Sprint 1

- **Resta Board-only.** Sherpa NON lo modifica, né in DRY_RUN né in LIVE.
- Sherpa **logga** `proposed_stop_buy_active = true` in `sherpa_proposals` quando `risk_score > 90`. Questo è il segnale "in un crash serio, avrei voluto bloccare i buy".
- `buy_pct` resta cappato a 3.0% max — non alziamo senza dati.
- **Decisione rimandata a Sprint 2:** dopo 1-2 settimane di dati DRY_RUN, se i trade durante risk_score > 90 si rivelano sistematicamente in perdita, promuoviamo `stop_buy_drawdown_pct` a parametro Sherpa e valutiamo se alzare il cap di buy_pct a 5.0% per più granularità nella zona difensiva.
...
- Calcola:
  ...
  - `speed_of_fall`: accelerazione della caduta. Definizione: il calo nell'ultimo terzo della finestra 1h (ultimi 20 min) è ≥ 1.5x il calo medio dell'intera finestra. Esempio: BTC -3% nell'ora, ma -2.5% concentrato negli ultimi 20 minuti → accelerating = true. Calcolo: `abs(change_last_20min) >= 1.5 * abs(change_1h / 3)`
...
**NOTA: `opportunity_score` in Sprint 1 è SOLO loggato.** Viene calcolato e scritto in `sentinel_scores` e `sherpa_proposals` per analisi futura, ma Sherpa NON lo usa per decidere i parametri. In Sprint 1 le decisioni si basano esclusivamente su `risk_score`. L'opportunity_score diventerà attivo in Sprint 2 quando avremo il regime detection (es. mercato bullish + alta opportunità → allarga sell_pct ancora di più). CC non deve inventare logica basata su opportunity_score.
...
1. **Base layer (regime):** definisce i parametri "a riposo" in base al regime di mercato. In Sprint 1 il regime è hardcoded a `neutral`. In Sprint 2 il regime verrà dal slow loop (Fear & Greed + CMC).
2. **Adjustment layer (fast signals):** modifica i parametri base in base ai segnali veloci (BTC price action + funding). Questo è l'unico layer attivo in Sprint 1.

**Parametro finale = base(regime) + adjustment(fast_signals)**, clampato ai range assoluti.

#### Base layer — tabella regime (Sprint 1: sempre `neutral`)
...
| **Neutral** | **1.0%** | **1.5%** | **1.0h** | **Sì (hardcoded)** |
...
#### Adjustment layer — delta da fast signals (Sprint 1: attivo)

| Segnale fast | Δ buy_pct | Δ sell_pct | Δ idle_reentry |
|-------------|-----------|-----------|----------------|
...
I delta si sommano tra loro. Esempio: BTC -5% in 1h + funding > 0.05% + speed accelerating → Δ buy_pct = +1.0 + 0.4 + 0.3 = +1.7%.  
...
In Sprint 2, basterà passare il regime reale dal slow loop invece di `"neutral"`. Zero refactoring su Sherpa.
...
- [ ] Calcola parametri come `base(regime) + Σ delta(fast_signals)`, clampati ai range assoluti
- [ ] In Sprint 1 il regime è sempre `neutral` (base hardcoded); le altre righe della tabella regime esistono nel codice ma non vengono usate
