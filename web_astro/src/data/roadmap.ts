  ...
  phases: [
    ...
    },
    {
      ...
      tasks: [
        ...
        { text: "Sentinel Sprint 2: slow loop (Fear & Greed + CMC dominance + regime detection)", status: "done", who: "AI", comment: "Session 78 (2026-05-14). Brief 77b SHIPPED in modular pattern (5 new files + 2 surgical edits, no monolith). Slow loop runs every 4h: fetches F&G (free) + CMC global metrics (paid, CMC_API_KEY in config/.env) → determines one of 5 regime buckets (extreme_fear / fear / neutral / greed / extreme_greed) → writes sentinel_scores row with score_type='slow'. Sherpa now reads regime dynamically via regime_reader instead of hardcoded 'neutral'. Test suite 37→85 (+48 new). CMC data logged but not yet used in regime calc (reserved for Sprint 2.5). Mapping inverted from brief: extreme_fear = opp=80/risk=20 (panic = buy zone). Mac Mini restart pending Max revision. Report: report_for_CEO/2026-05-14_s78_sentinel_sprint2_slow_loop_report_for_ceo.md." },
