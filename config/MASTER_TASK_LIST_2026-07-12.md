...
|---|---|---|---|---|
| K.1 | **CUTOVER Kraken — Fasi 0-4 (Max, 11-lug)**. Chiavi API ✅ (Withdraw OFF). **Fase 0 ✅ (11-lug)**: plumbing check 18/18 — **fee taker reale 0,80% tier-0 → 1,6% a giro** (Modello B maker da ri-esaminare in Board pre-deployment). **Fase 1 ✅ SHIPPED S118 (11-12 lug)**: colonna `venue` per-riga in bot_config + cablaggio hot-path via factory + `fee_rate` dinamico live + floor `avg×(1+margine+2×fee)` + fix contabile fee-in-quote (avg/cash/replay) + hands-off Sherpa su righe kraken + gate `ALLOW_REAL_MONEY` + bonifica cycle-fetch sito + disclaimer-toggle spento (anticipa parte di K.3) + **prova generale validate via client 28/28 sul Mini**. Invariante binance: zero diff (290 test). Piano `config/2026-07-11_S118_piano_kraken-fase1.md`. **Fase 2 (decide Max)**: finestra coordinata — insert righe kraken (parametri da chiudere, nodo 5) + is_active flip + disclaimer on + `ALLOW_REAL_MONEY=true` + restart + $100. **Fase 3**: collaudo BTC→SOL→BONK. **Fase 4**: deployment | Fase 2: runbook + decisione Max | CC + Max | **Fase 1 ✅ · Fase 2 attende Max** |
