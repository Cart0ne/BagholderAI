...
**Data:** 18 giugno 2026 (creata) · **Ultimo aggiornamento:** 1 luglio 2026 (post S114 + fix X-poster)
...
| 2.6 | **🆕 Blog post sul grid-regime backtest** — raccontare il verdetto "**il grid è un ammortizzatore di volatilità, non un motore**": batte hold solo nel laterale vero e di poco, cattura ~15% del rialzo / ~76% del ribasso. L'onestà è il differenziante narrativo | Da scrivere, **dopo 2.7** (serve il dataset completo) | CC (draft) + CEO/Max | usa i dati backtest S113 |
| 2.7 | **🆕 Estendere il backtest a SOL e BONK** — finora fatto **solo BTC** (3 regimi). Rifare su SOL e BONK con lo stesso harness (`scripts/backtest/`, fee Kraken, output gitignored) → chiude i backtest e alimenta 2.6 | — | CC | chiude i backtest |
| 2.8 | **🆕 Campagna X "Fails & Masterpoints"** (con CEO) — setacciare **TUTTE le sessioni + i diari**, estrarre i **fallimenti** autentici e i **colpi vincenti** ("masterpoint"), e trasformarli in post X che si chiudono con una **domanda alla community** per generare engagement. Es.: *"Anche a voi l'AI sbaglia l'analisi dei numeri? Quale state usando?"*. Materiale tutto interno (diary Supabase + transcript sessioni); sfrutta il listener `/approve` appena riparato | CEO (voce/cura) + CC (mining sessioni/diari) + Max (/approve) | tracciata qui perché **il CEO ha memoria corta** tra sessioni |
