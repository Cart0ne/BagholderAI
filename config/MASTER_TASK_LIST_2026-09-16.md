...
>
> **Ordine dei lavori vigente: [`config/SEQUENZA_post-golive_v1.md`](SEQUENZA_post-golive_v1.md)** (Board+CEO, 09-ago) — R.1 riconciliazione → X.1 XRP maker fee → Sentinel/NewsKeeper. **R.7 (S127) è candidata a entrare PRIMA di R.1**: senza di lei il denaro reale resta fermo e cieco per giorni senza che nessuno lo sappia — decisione di Max/CEO.
...
| ↳ R.1 note S127 | **Cosa il ledger Kraken contiene oltre ai trade** (letto in sola lettura il 16-set, autorizzazione Max): (a) **staking SOL automatico** (Auto Earn, accredito settimanale sabato 03:17 UTC, Kraken trattiene ~30%) — netto 0,0007659 SOL = esattamente il `wallet_surplus` del boot reconcile SOL; **Max: resta attivo** → R.1 deve classificare `type=staking` come movimento noto, non come divergenza; (b) **depositi** EUR con fee 2,4% + conversioni EUR→USD (`receive`/`spend` o `trade` su EUR/USD); (c) differenze trade DB↔ledger **sotto il centesimo** (il DB ricava `cost` da prezzo medio arrotondato a 2 decimali). **Tecnica**: leggere con `KrakenClient().raw` (nonce al microsecondo come i bot, altrimenti `Invalid nonce`) e **paginare** (`Ledgers` dà max 50 righe). Esito del confronto a mano del 16-set: BTC, SOL e USD **tornano**. Idea: chiave Kraken **solo-lettura** dedicata per R.1 | — | CC | 📝 input per R.1 |
| **R.7** | **🆕 Mac Mini autonomo (S127)** — dal **5 al 14 settembre** i bot sono rimasti accesi ma ciechi (il Mini si era **staccato dal Wi-Fi**, casa online: Max raggiungeva Home Assistant e il Mac mini server, non il Mini; fix = toggle Wi-Fi a mano), poi fermi fino al 16 dopo l'**aggiornamento macOS installato a mano da Max** (14-set 16:42), senza rilancio dei bot. **Nessun allarme è arrivato**: Telegram, `/admin` e publisher girano tutti sul Mini. Piano (da discutere, niente codice ancora): **(1)** LaunchAgent "avvio bot al login": aspetta 5 min → controlla Archivio montato + rete (Kraken + Supabase) + orchestrator non già vivo + assenza file "non avviare" → lancia orchestrator (flag vigenti) + NewsKeeper v2; se manca la rete riprova invece di partire cieco; niente KeepAlive sui bot (respawn a denaro reale = decisione separata). **(2)** LaunchAgent "watchdog Wi-Fi" ogni 2 min: router 192.168.1.254 + internet; se router irraggiungibile per 3 controlli → Wi-Fi off/10s/on (max 1 ogni 30 min), altrimenti solo log. Test con Max vicino al Mini (spegnere il Wi-Fi taglia SSH). **(3)** Watchdog **esterno** (proposta: controllo programmato GitHub che fallisce se i bot non scrivono a DB da X min → email automatica di GitHub; niente Telegram). **Risposte di Max (16-set)**: **FileVault resta** (opzione A); **aggiornamenti macOS: solo download, installazione a mano di Max con password** — ⚠️ il file di preferenze del Mini legge però `AutomaticallyInstallMacOSUpdates = 1`, da ricontrollare nel pannello; il riavvio del 14-set è stato **il suo aggiornamento manuale**, dopo il quale nessuno ha rilanciato i bot; **`pmset autorestart`**: Max aggiorna a **macOS 27** e poi cerca l'opzione nelle impostazioni (Energia). **Conseguenza chiave**: con FileVault + aggiornamenti manuali, il punto (1) copre esattamente il caso del 14-set (Max digita la password → login → dopo 5 min i bot ripartono da soli); resta scoperto solo il riavvio senza nessuno davanti (blackout elettrico). **Ancora da spiegare/decidere**: watchdog esterno (3) e priorità R.7 vs R.1; regola CLAUDE.md §5 per l'avvio automatico. **Tecnica**: launchd non può scrivere log su `/Volumes/Archivio` (lezione `/approve`) → log in `~/Library/Logs`; Automator/Shortcuts scartati (niente log, pensati per uso interattivo) | Il denaro reale è rimasto **11 giorni** senza operare e nessuno lo sapeva. È la stessa classe di R.1: la differenza fra "funziona" e "sappiamo che funziona" | CC + Max | 🆕 **PIANIFICATA** (S127) — se ne parla un altro giorno |
...
| ✅ **[S127] Umami API 401 — CHIUSA: Umami esce dalle fonti dell'audit A3.** Max ha verificato (16-set): le API key di Umami Cloud richiedono un **piano a pagamento**. **Sostituti**: GSC + Bing (connettori già attivi) + **Vercel Web Analytics** (script servito dal sito, ma l'API dati risponde 404 "Web Analytics not found" sul team Hobby → capire se si legge solo dal pannello). **Lavoro residuo (CC, piccolo)**: togliere Umami da `scripts/marketing_data_refresh.py`, aggiornare `docs/analytics-stack.md` e `audits/DATA_CAVEATS.md` (fatto S127); **testo nuovo del prompt Cowork A3** da preparare per Max (lo re-incolla lui nella UI) | Med | CC + Max | 🟡 decisa, esecuzione in coda |
| ✅ **[S127] Transcript Claude Code cancellati dopo 30 giorni — CHIUSA, si lascia così** (Max, 16-set: *"adesso lo so e va bene così, l'importante è che non perdi tu contesto"*). Il contesto sta nei file di stato e nella memoria, non nei transcript | — | — | ✅ chiusa |
...
*Aggiornato: CC, 16 settembre 2026 (**S127**) — ripartenza dopo il blackout Wi-Fi del Mini (5→14 set) e l'aggiornamento macOS del 14-set: bot fermi/ciechi 11 giorni, riavviati su richiesta Max (SHERPA_TELEGRAM tolto). **Aggiunti**: R.7 Mac Mini autonomo (piano, decisioni aperte), note per R.1 dal ledger Kraken (staking SOL resta attivo — Max), bug Umami 401 da verificare e bug transcript a 30 giorni. Puntatore alla sequenza lavori (`config/SEQUENZA_post-golive_v1.md`) in testa. File **rinominato** `2026-08-07` → `2026-09-16`.*
