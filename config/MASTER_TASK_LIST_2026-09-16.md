...
| **R.7** | **🆕 Mac Mini autonomo (S127)** — dal **5 al 14 settembre** i bot sono rimasti accesi ma ciechi (il Mini si era **staccato dal Wi-Fi**, casa online: Max raggiungeva Home Assistant e il Mac mini server, non il Mini; fix = toggle Wi-Fi a mano), poi fermi fino al 16 dopo un **aggiornamento macOS automatico** (14-set 16:42). **Nessun allarme è arrivato**: Telegram, `/admin` e publisher girano tutti sul Mini. Piano (da discutere, niente codice ancora): **(1)** LaunchAgent "avvio bot al login": aspetta 5 min → controlla Archivio montato + rete (Kraken + Supabase) + orchestrator non già vivo + assenza file "non avviare" → lancia orchestrator (flag vigenti) + NewsKeeper v2; se manca la rete riprova invece di partire cieco; niente KeepAlive sui bot (respawn a denaro reale = decisione separata). **(2)** LaunchAgent "watchdog Wi-Fi" ogni 2 min: router 192.168.1.254 + internet; se router irraggiungibile per 3 controlli → Wi-Fi off/10s/on (max 1 ogni 30 min), altrimenti solo log. Test con Max vicino al Mini (spegnere il Wi-Fi taglia SSH). **(3)** Watchdog **esterno** (proposta: controllo programmato GitHub che fallisce se i bot non scrivono a DB da X min → email automatica di GitHub; niente Telegram). **Decisioni aperte per Max**: FileVault (A tenere + login a mano · B togliere + auto-login · C verificare sblocco remoto su macOS 26); `pmset autorestart` (ripartenza dopo blackout elettrico, oggi 0); **aggiornamenti macOS automatici** (oggi ON: hanno causato i riavvii del 21-lug e del 14-set) → proposta: solo download, installazione quando decide Max; regola CLAUDE.md §5 per l'avvio automatico. **Tecnica**: launchd non può scrivere log su `/Volumes/Archivio` (lezione `/approve`) → log in `~/Library/Logs`; Automator/Shortcuts scartati (niente log, pensati per uso interattivo) | Il denaro reale è rimasto **11 giorni** senza operare e nessuno lo sapeva. È la stessa classe di R.1: la differenza fra "funziona" e "sappiamo che funziona" | CC + Max | 🆕 **PIANIFICATA** (S127) — se ne parla un altro giorno |
...
| **🆕 [S127] Umami API 401 — "Invalid API key."** La chiave è identica nelle 3 copie (`config/.env.marketing` MBP + Mini + `bagholderai-audits/marketing/`, creata ~30-mag). La premessa di luglio *"API key solo per piani a pagamento"* (`audits/DATA_CAVEATS.md` §3) **non è confermata** dalla documentazione ufficiale (docs.umami.is → Settings → API keys, nessun vincolo di piano citato). Max, prima di declassare definitivamente: *"vorrei capire cosa non funziona"*. **Prossimo passo (Max, 2 min)**: Umami Cloud → profilo → Settings → API keys → se la sezione c'è: nuova chiave nelle 3 copie + test CC; se manca / chiede upgrade: vincolo confermato → correggere il prompt Cowork A3 (che oggi non legge `DATA_CAVEATS.md` e segnala CRITICAL ogni mese, minacciando REJECTED) | Med (audit A3 cieco sul funnel da 3 mesi) | Max → CC | 🆕 aperto |
| **🆕 [S127] Le trascrizioni delle sessioni Claude Code si cancellano dopo 30 giorni** (`cleanupPeriodDays`, default 30, non impostato né sul MBP né sul Mini). Al riavvio del 16-set, dopo 38 giorni di pausa, sono sparite tutte le sessioni precedenti al ~17-ago; nessun Time Machine sul MBP → **non recuperabili** (il lavoro è salvo in git/stato/report). Sopravvivono S125/S126 (7-9 ago) e una breve del 26-ago, **in scadenza ~25-set**. Fix = `"cleanupPeriodDays": 3650` in `~/.claude/settings.json` su entrambe le macchine | Med (memoria `feedback_check_past_sessions` dipende dai transcript) | Max decide → CC | 🆕 aperto |
