...
| 4.11 | ~~Investigare recalibrate-on-restart (buy_pct cambia al boot)~~ | ✅ **CHIUSA 2026-06-29 (S111, decisione Max)** — indagine su `sherpa_proposals`+`bot_config`: i salti di `buy_pct` al boot erano **reali a maggio** (05-10/05-11: tutti e 3 i coin saltavano nello stesso secondo, valore ≠ proposta Sherpa con cooldown attivo = config_sync al boot, non Sherpa). **Causa**: Sherpa era DRY_RUN → non scriveva `bot_config` → al boot il bot leggeva valori stale/divergenti. **Risolto da S102b (2026-06-11, Sherpa GO LIVE)** + parametri protettivi Board S103a: Sherpa ora scrive `bot_config` (fonte unica) → al boot rilegge il valore vivo, niente revert. **Evidenza**: `buy_pct` costante 2.5/3.0/3.0 dall'11-giu attraverso 4 restart (25/27/28-giu×2); riga BTC `bot_config` non riscritta dal 12-giu. NON collegato al bug realized avg-cost (quantità diverse). Residuo opzionale: leggere una volta i log di boot Mac Mini per escludere un flicker in-memory non persistito. |
